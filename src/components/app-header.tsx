...
import {
  LayoutDashboard, Users, UserCog, Search, Bell, ChevronDown, Leaf,
...
const NAV_ITEMS = [
  ...
  { href: '/billing', label: '收费', icon: DollarSign },
  { href: '/consultation/manage', label: '问诊', icon: MessageSquare },
  { href: '/account', label: '设置', icon: UserCog },
] as const;
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:block">
            ...
            <div className="text-[10px] text-muted-foreground leading-tight">中医诊所管理系统</div>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-1 shrink-0 ml-auto">
          <button type="button" className="relative w-9 h-9 rounded-md hover:bg-surface-container flex items-center justify-center text-muted-foreground transition-colors" aria-label="通知">
            <Bell className="w-5 h-5" />
            <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-error" />
          </button>
          <button type="button" className="ml-1 flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-surface-container transition-colors">
            <div className="w-8 h-8 rounded-full bg-primary-container text-primary flex items-center justify-center text-sm font-bold">王</div>
